...
function createInternalAPI() {
  ...
  const changeListenerRegistryLegacy = new Set();
   ...
   */
  const namespaceToLayerCloseListenerRegistry = new Map();
  ...
  function increment() {
    if (!fg('platform_dst_open_layer_observer_close_layers')) {
      ...
      // Using `Array.from` to ensure we iterate over a stable list - e.g. in case a listener adds to the registry while we are
      // iterating over it.
      Array.from(changeListenerRegistryLegacy).forEach(listener => listener({
        count: openLayerCountLegacy
      }));
      return;
    }
  }
